...
| 14 | Calls into `packages/guidewire-client/**`'s write-shaped methods (HTTP `POST`/`PUT`/`PATCH`/`DELETE`) from **any** module — including `packages/harness/**` itself — outside an `execute()` callback | The harness's own read into `packages/guidewire-client/` is a layering exception (per [05-TECHNICAL-SPEC § 2.1](./05-TECHNICAL-SPEC.md#21-package-contracts-allowed-imports--public-api) `packages/harness/` row's *"read-only — write injection is the sole exception"* note); a future contributor adding a write call inside `packages/harness/src/foo.ts` outside `execute()` would silently regress the [D-006](../004-DR-DEC-architecture-decisions.md#d-006--hard-rule-no-audit--no-write) write-gate guarantee. Row 1 covers `servers/**`; this row closes the harness-internal loophole flagged in [03-AR F-2](./audits/03-AR-architecture-review.md#f-2). The manifest-level corollary (`requiresHarnessExecute: true` for `draft_only` / `approved_execute`) lives at [02-PRD § 3.0](./02-PRD.md#30-tool-manifest-contract). | depcruise + AST call-site rule per [009 § 8.2](../009-DR-MEMO-harness-runtime.md#82-ci-enforcement-architecture-rule). REFUSE |
...
| Stale read after write (per [GA-6](./audits/10-GA-guidewire-api-review.md#f-6) + [008 § 9](../008-DR-MEMO-guidewire-api.md)) | L4 (Cloud API write returns 202 + Async job ID; immediate GET returns pre-write state) | Per `confirmWrite(plan, opts)` strategy declared in `profile.yaml.write_confirmation.strategy`: (a) `trust_202` — emit `execute.completed` on 202; (b) `poll_async_job` — poll `/async/v1/jobs/{id}` to terminal; (c) `wait_for_app_event` — wait for the matching App Event before emitting. Default OSS demo = `trust_202`. Full contract at [02-PRD § 5.4 "Read-after-write consistency"](./02-PRD.md#54-execute--side-effect-with-idempotency) and signature at [05-TECHNICAL-SPEC § 3.4](./05-TECHNICAL-SPEC.md#34-execute--side-effect-with-idempotency-p1-librarian-correction-inline). | Strategy (a): operator's read-back may show stale state for several seconds — operator's mental model accommodates this; Strategy (b)/(c): the harness waits for terminal state before returning, so the operator's immediate read-back is durable |
